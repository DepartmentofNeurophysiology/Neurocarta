function [leftplot,rightplot,smallplots] = crosscorrelatemaps1(group1,group2,labels,hemi)
%% Compare two experiments in a layered diagram
% 
% crosscorrelatemaps1(group1,group2,[labels],[hemi])
% 
% Input:
...
rightplot = subplot(3,4,[3 4 7 8]);
smallplots = [subplot(3,4,11) subplot(3,4,12)];

% Set position of the subplots
margin = 0.03; % Margin space between subplots
leftplot.Position = [margin 0 0.5-2*margin 1];
rightplot.Position = [0.5+margin margin+1/3 0.5-2*margin 0.6-2*margin];
for i = 1:2
    smallplots(i).Position = [0.5+margin+0.25*(i-1) margin 0.25-2*margin 0.3-2*margin];
end

%% Draw layered diagram on the left
layereddiagram(leftplot,data);

%% Draw scatter plot on the right
scatterplot(rightplot,data);

%% Draw subplots with statistical information
correlationplots(smallplots,data);
